  ...
  return (
    <header className="pb-[30px]">
      <section className="px-[128px] py-[13px] flex items-center justify-between bg-[#EBEFF3]">
        <div className="flex items-center gap-[30px]">
          <button className="flex items-center gap-[10px]">
            <HeaderLocationIcon />
            <span className="text-[#545D6A] text-[14px] font-normal leading-[130%]">
              Tashkent
            </span>
          </button>
          <div className="flex items-center space-x-[15px]">
            {HeaderINfoS.map((item) => (
              <Link
                className="text-[#545D6A] text-[14px] font-normal leading-[130%]"
                href={item.path}
                key={item.id}
              >
                {item.title}
      ...
      <div className="px-[128px] py-[25px] flex items-center justify-between">
        <div className="flex items-center gap-[35px]">
          <Link
            className="flex items-center text-[#134E9B] text-[36px] font-black leading-[42px]"
            href={"/"}
          >
            <span>
              <Logo />
          ...
          <div className="flex items-center gap-[10px]">
            ...
            <label className="rounded-[6px] bg-[#EBEFF3] pl-[26px] flex items-center">
              <input
                className="w-[262px] text-[rgba(0, 0, 0, 20%)] text-[13px] font-normal leading-[15px] bg-transparent outline-none"
                type="text"
                placeholder="What are you looking for?"
                autoComplete="off"
              />
              <button className="bg-[#134E9B] rounded-[6px] py-[14px] px-[20px]">
                <HeaderSearchIcon />
              </button>
            </label>
          </div>
        ...
        <div className="flex items-center space-x-[15px]">
          ...
          >
            {isLogin == "login" ? (
              ...
              <CreateUsersInput setIsLogin={setIsLogin} />
            )}
          </CustomModal>
        </div>
      </div>
      <section className="flex items-center justify-between px-[128px]">
        {categories.map((item) => (
          <span
            className="text-[#545D6A] text-[16px] font-normal cursor-pointer leding-[21px]"
            key={item.id}
          >
            {item.name}
          </span>
